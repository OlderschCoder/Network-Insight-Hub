...
**Purpose:** One document for **physical locations** (map + remote) and **how they relate** to the documented network (FortiGate, Azure, Cisco aa144, Aruba). Use for **engineering analysis**, **enterprise diagramming** (which layer feeds which drawing), and **strategic planning** when paired with **`INDEX.md`** and the sources below.

**Master navigation:** **`INDEX.md`** — reading order, diagram layers, Azure export map, planning checklist.

**Sources:** Campus map (isometric main campus), stakeholder notes in **`BUILDING_LIST.md`**, switch configs **sw-aa144-a48** / **sw-aa144-a24**, FortiGate/Azure analysis, **`NETWORK_OVERVIEW.md`**, **`BUILDING_SWITCH_TOPOLOGY.md`** (ports/OSPF), **`SWITCH_INVENTORY.md`** (live IPs), **`azure-export-*`** JSON (Azure subscription snapshot).

**Limitations:** Not a certified as-built. VLAN names may lag renamed facilities (e.g. **ColvinBuilding** vs **Calvin Allied Health**). Confirm critical rows with facilities + live `show vlan`. Azure JSON is **point-in-time** — re-run **`azure_visualizer.ps1`** for current state.

### How to use this doc for diagramming & planning

| Need | Use sections |
|------|----------------|
| **Executive / campus map** | **§1** (buildings), **§4.4** (Azure VMs = AD / file / print), **§§5.1–5.2** Mermaid (map + L3 path) |
| **Distribution + routing** | **§2** (voice SVIs, OSPF **/30**), then **`BUILDING_SWITCH_TOPOLOGY.md`** for **exact ports** |
| **Access switches + mgmt IPs** | **`SWITCH_INVENTORY.md`**, **`BUILDING_SWITCH_TOPOLOGY.md`** §4 |
| **Cloud / hybrid overlay** | **§4–5** here + **`NETWORK_OVERVIEW.md`** §4 + **`azure-export-*`** (`20` VNets/subnets, `21` public IPs, `22` LB, `23` NAT, `27–29` VPN, `40`/`50` VMs/NICs) — **same diagram family** as campus |
| **Risk / remediation backlog** | **§6** + **`INDEX.md`** §7 checklist |
...
| **Azure** | **S2S-OnPrem** **Connected**; **OnPrem-LNG** includes **172.25.0.0/21** and main campus ranges. **VMs, public IPs, load balancers, and NSGs** are **production extension** of this network (not a separate documentation silo) — diagram them with FortiGate and campus paths. |
...
### 4.4 Core services in Azure (VMs the sites connect to)

**Diagram rule:** Treat **virtual machines** as the **servers the network delivers traffic to** — same drawing as **Main**, **West**, **Epworth**, and **FortiGate**. Campus and remote clients reach **AD**, **file shares**, **print**, and related roles over **LAN / VPN / IPsec** into **Hybrid-VNet** (and related RGs), not as an afterthought box labeled only “cloud.”

Names below come from subscription export **`azure-export-20260417-212136`** / **`40-vm-list-detail.json`** (re-run **`azure_visualizer.ps1`** to refresh). **Logical role** is inferred from hostname and common IT usage — **confirm** in your CMDB / server inventory.

| Azure VM (name) | Logical role (typical) | Notes |
|-----------------|------------------------|--------|
| **AD-SC01** | Domain controller / AD DS | **HYBR0D-NETWORK**; private IP **10.0.0.34** in export. |
| **AzADC001** | Domain controller–class workload | **SCCC_Domain_services** RG. |
| **EntraCloudConnect** | Entra ID (Azure AD) hybrid connectivity | Sync / hybrid identity path between cloud and on-prem AD. |
| **CiscoDirectoryConnector** | Directory connector (Cisco stack) | Pairs with UC/collab identity flows. |
| **MFS0** | **Primary file server** (often spoken as “MSF0”) | Multiple data disks in export. |
| **SCCCMFS01** | File / shares | Second file-server VM in **HYBR0D-NETWORK**. |
| **PRNTSP2** | **Print server** | Central spooler / queues for clients. |
| *(managed service)* | **Azure AD Domain Services** | **Microsoft.AAD/DomainServices** in **SCCC_Domain_services** — not a VM you patch like **AD-SC01**, but part of the **same identity diagram**. |

Other VMs in **`50-graph-vms.json`** (e.g. **WSUS**, **WEB**, **XMAIL16-New**, lab/test) should appear on **detailed** Azure topology drawings; put **identity + file + print** on **every** high-level enterprise diagram alongside buildings and firewall.

---

## 5. Diagram — locations + network paths (hybrid)

**Scope:** After moving workloads to Azure, **overall network diagrams** should include **named production VMs** (**§4.4**) in the **same figure** as **Main campus**, **West**, **Epworth**, and **FortiGate** — plus VNets, subnets, **NICs**, **public IPs**, **LB/NAT** from **`azure-export-*`** (`20`, `21`, `22`, `23`, `40`, `50`). Below is a **logical** view (not every VM).

High-level: **main campus buildings** share **aa144 → FortiGate LAN**; **Epworth** and **West** attach through the **core / VPN** paths already documented; **Azure** hosts **AD, file, print**, and other servers that **all sites use** over **S2S** and (where applicable) **public** edges.

**L3 reality (not L2):** **VLANs do not extend to Azure.** User / printer / lab traffic stays in **on‑prem L2 VLANs** until **L3 routing** moves it toward the **FortiGate** and **IPsec** to **Hybrid‑VNet**. On the Azure side, **VMs attach to Azure subnets** (e.g. **Hybrid_default** **10.0.0.0/25** per **`20-network-vnet-list.json`**); **NIC private IPs** (e.g. **AD‑SC01** **10.0.0.34** per **`40-vm-list-detail.json`**) are **reachable** from on‑prem because **both sides advertise / learn prefixes** (**OnPrem‑LNG**, FortiGate **InternalAzure** object, **UDR** toward **VirtualNetworkGateway** where applicable — full lists in **`NETWORK_OVERVIEW.md`** §2 and Azure export). Diagrams below are **complementary**: **map + sites**, then **routing integration**.

### 5.1 Diagram — sites, edge, and Azure (map view)
...
flowchart TB
  ...
  subgraph Cloud["Azure centralus production"]
    HV["Hybrid-VNet and VPN GW"]
    SN["Azure subnets e.g. Hybrid_default 10.0.0.0/25"]
    DC["DC VMs: AD-SC01, AzADC001, EntraCloudConnect"]
    FSrv["File: MFS0, SCCCMFS01"]
    PRT["Print: PRNTSP2"]
    Edge["Public IP, LB, NAT"]
    HV --- SN
    SN --- DC
    SN --- FSrv
    SN --- PRT
...
Link labels are omitted above for **Mermaid portability** (some parsers mishandle `|` next to hyphens in labels). **IPsec** names (**West**, **Azure‑SCCC2**) and **S2S** (**OnPrem LNG** prefixes) are documented in **`NETWORK_OVERVIEW.md`** and FortiGate export.

### 5.2 Diagram — how VLAN / site subnets reach Azure VM LANs (L3 path)

Use this for **“comprehensive integration”**: same traffic path your **VLAN SVIs** and **West** prefixes use to hit **server VMs** on **Azure subnet NICs** (not a flat line; **routing + tunnel + policy**).

```mermaid
flowchart LR
  subgraph OnPrem["On-premises L3"]
    V["Building VLAN SVIs user server printer Wi-Fi VLANs"]
    OSPF["Nexus aa144 OSPF transit to FortiGate path"]
    V --> OSPF
  end

  subgraph WestNet["West site"]
    WV["172.25.x VLAN910 and related"]
  end

  FGT2["Fortigate1-Sccc"]
  IPSEC["IPsec to Azure e.g. Azure-SCCC2"]
  VNG["Azure VPN Gateway Hybrid-VNet"]
  AZSUB["Subnet Hybrid_default 10.0.0.0/25"]
  VMN["VM NICs e.g. AD-SC01 10.0.0.34, MFS0, PRNTSP2"]
...
**Reverse / eastbound:** Azure **UDR** and **S2S** advertise **on‑prem prefixes** (see **OnPrem‑LNG** in **`27–29`** exports) so **VMs initiate** or **reply** back to **VLAN** clients within those routes. **NSG** on subnets/NICs filter after routing — see **`25-network-nsg-list.json`**.

**Detail drill-down:** VM names, private IPs, **PIP** associations, and LB frontends are in **`40-vm-list-detail.json`**, **`50-graph-vms.json`**, **`21-network-public-ip-list.json`**, **`22-network-lb-list.json`**, **`23-network-nat-gw-list.json`** (per **`INDEX.md`** §4).
...
| File | Contents |
|------|----------|
| **`INDEX.md`** | **Master index:** audiences, diagram layers, Azure bundle map, planning checklist. **Start here** for new work. |
...
| **`NETWORK_OVERVIEW.md`** | Full technical notes, VLAN tables, Mermaid WAN diagram (FortiGate + Azure narrative). |
| **`Main_West_NETWORK_OVERVIEW.md`** | Legacy duplicate — align with **`NETWORK_OVERVIEW.md`** or retire. |
| **`AZURE_VISUALIZATION_CLI.md`** | **`azure_visualizer.ps1`** — subscription JSON exports for diagrams. |
| **`azure-export-*`** (folder) | Timestamped **`az`** output; see **`INDEX.md`** §4 for file roles. |
...
- Add a **PDF campus map** under `docs/` and link **`BUILDING_LIST.md`**.  
- Build a **spreadsheet** from **§1.6** (columns): start with **§2.2–2.3** on **`sw-aa144-a48`**, then add **data/voice subnets** from **`show ip interface brief`** on each **Aruba** stack listed in **`BUILDING_SWITCH_TOPOLOGY.md`** / **`SWITCH_INVENTORY.md`** (and IPAM).  
- Refresh **`azure-export-*`** on a **cadence** (e.g. quarterly) after **`azure_visualizer.ps1`**; archive zips for **strategic** reviews.  
