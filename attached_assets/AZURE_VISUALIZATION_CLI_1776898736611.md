...
**Repo context:** These exports label **Azure VMs, public IPs, load balancers, NAT, VNets, and NSGs** on **the same enterprise network diagrams** as on‑prem (post–cloud migration), not a siloed “Azure-only” drawing set. Combine with **`NETWORK_OVERVIEW.md`** (FortiGate alignment), **`SCCC_NETWORK_OVERVIEW_ALL_BUILDINGS.md`** §§5.1–5.2, and **`INDEX.md`** (diagram layers + file map).
