...
**Purpose:** Single matrix for **Nexus distribution** (**sw-aa144-a48** / **sw-aa144-a24**), **building-facing ports**, **OSPF routed links**, **live IP inventory**, and **`*.sccc.edu`** paths in **`c:\Code\Aruba`**. This is the **authoritative port/OSPF reference** for **enterprise L2/L3 campus diagrams** and **troubleshooting**; pair with **`SCCC_NETWORK_OVERVIEW_ALL_BUILDINGS.md`** for WAN/cloud context and **`INDEX.md`** for the full doc map.
...
### Using this document for diagramming & strategy

| Deliverable | Use these sections |
|-------------|---------------------|
| **Visio / Lucid “underlay”** (how buildings attach to core) | **§2–3** (Ethernet downlinks), **§4** (hostname ↔ file ↔ IP) |
| **Routing overlay** (OSPF p2p, not user subnets) | **§1** (Vlan611–628 **/30**) — label diagrams **“transit”** vs **user VLAN** |
| **Mgmt / monitoring map** | **`SWITCH_INVENTORY.md`** + **§4** |
| **Gap analysis** (naming, missing live, SWA-V201 vs m201) | **§5** |
| **Azure / hybrid (VMs, PIPs, LB, VNet)** | **Not drawn in port tables here** — add **`azure-export-*`** (`20`–`23`, `40`, `50`) **onto the same enterprise diagram** as this underlay (**`SCCC_NETWORK_OVERVIEW_ALL_BUILDINGS.md`** §§5.1–5.2, **`INDEX.md`** §3). |

**Refresh:** Re-export Nexus configs after change windows; diff **`sw-aa144-a48`** / **`a24`**; update port tables if descriptions change.
...
- **`INDEX.md`** — full documentation map (diagram layers, Azure exports, planning checklist).  
