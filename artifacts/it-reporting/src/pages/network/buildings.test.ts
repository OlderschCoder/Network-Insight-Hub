...
describe("shared Buildings and Monitoring campus-map entries", () => {
  it("includes the Mansions marker by default", () => {
    ...
    expect(entries.find(({ code }) => code === "MAN")?.match?.name).toBe("Mansions");
  });
  ...
  it("excludes a marker whose saved layout visibility is false", () => {
    ...
    expect(entries.find(({ code }) => code === "SLC")?.match?.name).toBe("Student Living Center");
  });
  ...
  it("gives an unlisted building a stable custom marker that can also be hidden", () => {
    ...
    const hidden = getCampusMapDisplayEntries(
    ...
    expect(hidden.some(({ buildingName }) => buildingName === "Warehouse Annex")).toBe(false);
  });
  ...
  it("restores valid default coordinates before showing a legacy hidden custom marker", () => {
    ...
    const hidden = mergeCampusMapLayoutRows(defaults, [{ code, visible: false }]);
    ...
    expect(Number.isFinite(shown[code].x) && Number.isFinite(shown[code].y)).toBe(true);
  });
  ...
  ])("does not duplicate the preferred canonical name %s as a custom marker", (name, expectedCode) => {
    const entries = getCampusMapDisplayEntries([building(name)]);

    expect(entries.find(({ code }) => code === expectedCode)?.match?.name).toBe(name);
    expect(entries.some(({ code }) => code.startsWith("CUSTOM:") && code.includes(encodeURIComponent(name)))).toBe(false);
  });
