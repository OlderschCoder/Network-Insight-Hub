...
const APP_USAGE_ENTRIES: {
  ...
  },
  {
    ...
    content:
      "Network Tools (/network/tools) is visible only to network-admin roles (CIO, network, network engineer). It includes the live FortiGate website-whitelist workflow and bounded script tools. Install Printer now loads the exact drivers registered on prntsp2.sccc.edu and creates or updates the shared RAW TCP/9100 queue through Fred after validating a private campus IPv4 address; it does not install or guess drivers. Other client-side generators still produce a downloadable PowerShell script to run on the target Windows machine. Day-to-day phone support for the wider IT team is under IT Apps > Cisco Calling (/it-apps/cisco-calling), not Network Tools.",
  },
  {
    category: "general",
  ...
  },
  {
    ...
    content:
      "Troubleshooting (/support) shows recent Zendesk activity and opens diagnostic workflows. Zendesk Monitor (/support/zendesk) polls the open conversation queue, shows the recent thread, asks Fred for one read-only reply draft, or uses Prepare open drafts to review up to 25 loaded open tickets. Batch review skips existing drafts, pending tickets, staff-last replies, internal-only activity, and escalation-only cases; it saves drafts but never sends them. Read the complete thread before drafting and do not repeat answered questions. Zendesk already supplies the requester's identity and email, so do not ask for either when present. For SCCC wireless, tell the requester to use their SCCC network username and password but never ask them to send, paste, or disclose a password or MFA code. SCCC IT can update Microsoft 365 licenses; ask only for missing application or entitlement details. The operator can edit, replace, clear, or explicitly approve an exact public reply before it is posted. Auto-send is off. The CIO and explicitly delegated supervisors can globally turn Fred drafting and Zendesk public replies on or off from the monitor; the API persists and enforces both controls and records who changed them, while manual escalation remains available. Disabling Fred blocks its internal notes, ticket updates, ticket creation, solves, and new Fred drafts. Disabling replies blocks every public-send path but still permits confirmed internal notes. If the persisted control state cannot be read, the API fails closed and performs no Zendesk write. The monitor can escalate a ticket to an active team member with an optional private handoff note after the exact assignment is confirmed. Fred can search and read tickets, create a ticket, edit its subject/status/priority/assignment, add confirmed public replies or internal notes, recommend or perform a confirmed escalation, and solve a specific confirmed list of tickets when Fred Zendesk actions are enabled. In Zendesk, 'close' should be treated as 'solve'; Zendesk automation applies the final irreversible Closed state later. Before any ticket write, Fred must show the exact ticket or ticket IDs and proposed change and receive explicit confirmation. An unbounded request such as 'close all tickets' is not enough—Fred must search, present the exact set, and ask for confirmation.",
  },
  {
    category: "general",
    title: "Using the Platform: Fred and Fred Memory",
    content:
      "Fred (/ai-report) can answer operational questions, guide users through the whole portal, list accessible to-dos, create or complete confirmed to-dos, let delegated team managers move confirmed to-dos between team members, work with confirmed Zendesk tickets, manage confirmed Post-Incident Reviews and weekly logs within the user's permissions, and let the CIO create, edit, or finalize confirmed weekly status reports. Fred Memory is persistent knowledge about the SCCC environment; every active entry is loaded into Fred's context. Users can search, filter, add, edit, and toggle memories; only the CIO can delete. An edited draft does not train Fred by itself: save a durable correction to Fred Memory, and put complete repeatable procedures in Process Library (/processes). Never store passwords or secrets in Fred Memory; the system blocks credential-like content.",
  },
  {
    category: "general",
    title: "Using the Platform: User Guide page",
    content:
      "The built-in User Guide at /user-guide contains the full written, step-by-step guide to signing in, the four Home workspaces and persistent sidebar switcher, Status, To-do List, Completed Work, Weekly Log, Risks & Issues, Post-Incident Reviews, Network and IT Tools, Troubleshooting, Weekly Reports, Projects, Department Goals, Fred, and Admin. Quick Start (/quick-start) is the concise 10-minute orientation. Both are available from Troubleshooting. When a user wants help, give the immediate click-by-click steps, link to Quick Start for orientation, and link to User Guide when full detail is useful.",
  },
  {
    category: "general",
