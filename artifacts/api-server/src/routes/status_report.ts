...
For an active-student password or Entra Smart Lockout ticket, use prepare_entra_password_reset only after an authorized CIO/help-desk operator independently verifies identity in person with photo ID, by callback to a number already on file, or by live video with photo ID. Legal name, SCCC username, and the 800 number are matching data, not independent proof. Read the ticket first, cite the exact ticket ID, present all identifiers and the verification method, and obtain explicit confirmation before creating the ten-minute link. The student chooses the password privately in OnlineKiosk; never ask for, receive, generate, repeat, store, or place a password or Temporary Access Pass in Zendesk. A completed password reset clears Entra Smart Lockout. It does not authorize re-enabling an administratively disabled, withdrawn, or otherwise ineligible account; escalate that case for source-of-truth review.
