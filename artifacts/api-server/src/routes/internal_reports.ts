...
import {
  buildEupAcceptedManifest,
  signEupAcceptedManifest,
...
const studentNotificationSubject = "SCCC password changed through OnlineKiosk";
...
async function persistOnlineKioskActivity(
  row: Record<string, unknown>,
): Promise<void> {
  const activityPath =
    process.env.ONLINE_KIOSK_ACTIVITY_PATH ||
...
router.post("/password-reset-report", async (req, res) => {
  ...
  const signatureText = String(
    req.header("X-SCCC-Report-Signature") ?? "",
  ).toLowerCase();
  ...
  const csvFileName =
    typeof req.body?.csvFileName === "string" ? req.body.csvFileName : "";
  const csvBase64 =
    typeof req.body?.csvBase64 === "string" ? req.body.csvBase64 : "";
  if (
    !subject.startsWith(subjectPrefix) ||
    subject.length > 180 ||
    html.length === 0 ||
    html.length > 262_144 ||
    !/^sccc-password-reset-log-[0-9]{8}-[0-9]{4}\.csv$/.test(csvFileName)
  ) {
  ...
  try {
    const keyPath =
      process.env.KIOSK_REPORT_KEY_PATH || "/var/lib/sccc-mfa/kiosk-device.key";
    ...
    logger.info(
      { recipient: reportRecipient, csvBytes: csv.length },
      "Sent kiosk password reset report",
    );
    res.json({ sent: true });
  } catch (error) {
...
router.post("/eup-alert", async (req, res) => {
  ...
  const signatureText = String(
    req.header("X-SCCC-EUP-Signature") ?? "",
  ).toLowerCase();
  const timestamp = Number(timestampText);
  const alertId =
    typeof req.body?.alertId === "string"
      ? req.body.alertId.trim().toLowerCase()
      : "";
  const severity =
    typeof req.body?.severity === "string"
      ? req.body.severity.trim().toLowerCase()
      : "";
  const subject = typeof req.body?.subject === "string" ? req.body.subject : "";
  ...
  if (
    ...
    !(
      subject.startsWith(eupAlertSubjectPrefix) ||
      subject.startsWith(eupRecoverySubjectPrefix)
    ) ||
    subject.length > 180 ||
    text.length === 0 ||
    text.length > 32_768 ||
  ...
  try {
    const keyPath =
      process.env.EUP_MONITOR_KEY_PATH || "/etc/sccc-eup/monitor.key";
    const key = await readFile(keyPath);
    ...
    logger.info(
      { alertId, severity, recipients: eupAlertRecipients },
      "Sent EUP provisioning alert",
    );
    res.json({ sent: true, alertId, recipients: eupAlertRecipients });
  } catch (error) {
    logger.error(
      { err: error, alertId, severity },
      "Failed to send EUP provisioning alert",
    );
    res.status(502).json({ error: "Mail delivery failed" });
  }
});
...
router.get("/eup-accepted-manifest", async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      res
        .status(401)
        .json({ error: "Invalid accepted-manifest authentication" });
      return;
    }
    const monitorPath =
      process.env.EUP_ACCEPTED_MONITOR_PATH ||
      "/var/lib/sccc-eup/accepted-student-monitor.json";
    const monitor = JSON.parse(await readFile(monitorPath, "utf8"));
    const bannerVerificationPath =
      process.env.EUP_BANNER_VERIFICATION_PATH ||
      "/var/lib/sccc-eup/banner-person-verification.json";
    const bannerVerification = JSON.parse(
      await readFile(bannerVerificationPath, "utf8"),
    );
    const responseTimestamp = Math.floor(Date.now() / 1000).toString();
    const responseTime = new Date(Number(responseTimestamp) * 1000);
    const manifest = buildEupAcceptedManifest(
      monitor,
      bannerVerification,
      responseTime,
    );
    const signed = signEupAcceptedManifest(
      key,
      timestampText,
      responseTimestamp,
      manifest,
    ...
    res
      ...
      .set({
        ...
        "X-SCCC-Manifest-Timestamp": signed.responseTimestamp,
        "X-SCCC-Manifest-SHA256": signed.bodySha256,
        "X-SCCC-Manifest-Signature": signed.signature,
      })
    ...
    logger.error(
      { err: error },
      "Failed to provide EUP accepted-student manifest",
    );
...
router.post("/eup-cloud-sync-heartbeat", async (req, res) => {
  ...
  const timestamp = Number(timestampText);
  const computerName =
    typeof req.body?.computerName === "string"
      ? req.body.computerName.trim()
      : "";
  const utc = typeof req.body?.utc === "string" ? req.body.utc.trim() : "";
  const serviceStatus =
    typeof req.body?.serviceStatus === "string"
      ? req.body.serviceStatus.trim()
      : "";
  const serviceStartType =
    typeof req.body?.serviceStartType === "string"
      ? req.body.serviceStartType.trim()
      : "";
  const traceUtc =
    typeof req.body?.traceUtc === "string" ? req.body.traceUtc.trim() : "";
  const agentVersion =
    typeof req.body?.agentVersion === "string"
      ? req.body.agentVersion.trim()
      : "";
  ...
  try {
    ...
    if (
      ...
      res
        .status(401)
        .json({ error: "Invalid Cloud Sync heartbeat authentication" });
      return;
    }

    const heartbeatPath =
      process.env.EUP_CLOUD_SYNC_HEARTBEAT_PATH ||
    ...
    const payload = {
      ...
      traceAgeSeconds: Math.max(
        0,
        Math.round((Date.now() - observedTraceUtc) / 1000),
      ),
      agentVersion,
      serviceBusConnections,
    };
...
router.post("/eup-ad-snapshot", async (req, res) => {
  ...
  const snapshotSha256 =
    typeof req.body?.snapshotSha256 === "string"
      ? req.body.snapshotSha256.toLowerCase()
      : "";
  const snapshotBase64 =
    typeof req.body?.snapshotBase64 === "string" ? req.body.snapshotBase64 : "";
  if (
    !Number.isSafeInteger(timestamp) ||
    Math.abs(Date.now() / 1000 - timestamp) > 300 ||
    !/^[0-9a-f]{64}$/.test(signatureText) ||
  ...
  try {
    ...
    if (
      ...
      snapshotBytes.length > 6_000_000 ||
      sha256(snapshotBytes) !== snapshotSha256
    ) {
    ...
    if (
      ...
      accounts.some(
        (account: any) =>
          ...
          !String(account?.UserPrincipalName ?? "")
            .toLowerCase()
            .endsWith("@sccc.edu") ||
          !String(account?.DistinguishedName ?? "")
            .toLowerCase()
            .includes(",ou=studentpopulation,ou=people,dc=sccc,dc=edu"),
      )
    ) {
    ...
    const snapshotPath =
      process.env.EUP_AD_SNAPSHOT_PATH ||
      "/var/lib/sccc-eup/ad-cohort-reconciliation.json";
    ...
    logger.info(
      { total: accounts.length, needReview: snapshot.NeedReview },
      "Stored EUP AD verification snapshot",
    );
    res.json({
      stored: true,
      total: accounts.length,
      generatedUtc: new Date(generatedUtc).toISOString(),
    });
  } catch (error) {
    logger.error(
      { err: error },
      "Failed to store EUP AD verification snapshot",
    );
...
router.post("/student-password-reset-notification", async (req, res) => {
  ...
  const onlineKioskCaller =
    isOnlineKioskVm(req) && /^10\.0\.0\.44(?::8080)?$/.test(host);
  ...
  try {
    ...
    logger.error(
      { err: error, recipient },
      "Failed to send OnlineKiosk password-reset notification",
    );
    res.status(502).json({ error: "Mail delivery failed" });
  }
});
...
router.post("/online-kiosk-reset-activity", async (req, res) => {
  ...
  const eventId =
    typeof req.body?.eventId === "string"
      ? req.body.eventId.trim().toLowerCase()
      : "";
  const utc = typeof req.body?.utc === "string" ? req.body.utc.trim() : "";
  const account =
    typeof req.body?.account === "string"
      ...
      : "";
  const sourceIp =
    typeof req.body?.sourceIp === "string" ? req.body.sourceIp.trim() : "";
  ...
  try {
    ...
    logger.error(
      { err: error, eventId },
      "Failed to record OnlineKiosk password reset activity",
    );
