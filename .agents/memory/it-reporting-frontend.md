...
- **Seed/login creds in `replit.md` can be stale.** Generic `*@sccc.edu` seed logins may 401 because the real active CIO account differs; verify the active account via the dev DB / admin rather than trusting the README. To log in for e2e, set a known bcrypt password on an existing dev test-CIO row (dev DB only).

- **`bcrypt` (in `api-server/auth.ts`) needs `@types/bcrypt` as a devDependency** or typecheck fails with TS7016. bcrypt is a native module — it isn't importable from the code-execution sandbox root; run any bcrypt hashing via `node` inside `artifacts/api-server`.

- **Zendesk integration: the auth-header code is correct; 401s come from the Zendesk account side.** A 401 "Couldn't authenticate you" on `/users/me` persists even with a freshly issued `ZENDESK_API_TOKEN`, which means the cause is external config (API token access disabled, or agent email/subdomain mismatch) — not a code bug. Don't keep re-requesting tokens; surface a graceful "not connected" UI and tell the admin to verify Zendesk API settings. The backend deliberately keeps HTTP 502 (not 401) for upstream auth failures so the SPA's auth handling isn't tripped into logging the user out; instead it returns a stable `code` field (`ZENDESK_AUTH` / `ZENDESK_NOT_CONFIGURED` / `ZENDESK_ERROR`) and never echoes the raw upstream body. The `ZendeskResolved` card classifies by that `code`, not by HTTP status.
