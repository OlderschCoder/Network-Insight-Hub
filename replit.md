...
- Env vars: `ENTRA_TENANT_ID`, `ENTRA_CLIENT_ID`, `ENTRA_CLIENT_SECRET`, `ENTRA_REDIRECT_URI`, **at least one of** `ENTRA_ALLOWED_GROUP_IDS` / `ENTRA_ALLOWED_APP_ROLES` (required — SSO fails closed without a gate), optional `ENTRA_ROLE_MAP_JSON`, `ENTRA_POST_LOGIN_REDIRECT` (override the SPA landing URL). These are normally **separate** from the `AZURE_*` service-principal secrets used for VM inventory. **Credential reuse:** if the dedicated `ENTRA_TENANT_ID`/`ENTRA_CLIENT_ID`/`ENTRA_CLIENT_SECRET` trio is unset, `getEntraConfig()` falls back to the `AZURE_TENANT_ID`/`AZURE_CLIENT_ID`/`AZURE_CLIENT_SECRET` trio (all-or-nothing per set — never mixed) so one Entra app registration can power both VM inventory and SSO. To reuse, the operator still must (1) add the Web redirect URI to that registration, (2) ensure delegated `openid`/`profile`/`email`/`User.Read` consent, and (3) set an access gate. `ENTRA_REDIRECT_URI` is always required (no fallback) and must exactly match the registered URI.
